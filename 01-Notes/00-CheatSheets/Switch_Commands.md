...
Command|Description
---|---
``S1#show running-config``|display the configuration currently active in RAM — this is the switch's live, working config, and may differ from what's saved to NVRAM if changes haven't been saved yet
``S1#show history``|displays the buffer of recently entered commands for the current session. Default buffer size is 10 lines — change it with ``terminal history size [lines]`` (or ``history size [lines]`` from a line configuration mode)
``S1#show interface [int-id]``|useful to detect errors or verify packets are being sent and received
``S1#show mac address-table``|display the switch's MAC address table — which MAC address was learned on which port, and which VLAN that port belongs to. Essential for tracing where a specific device is physically connected
...
``S1#show vlan``|display detailed VLAN information, including per-VLAN STP state, MTU, and port membership. For a shorter, more scannable view, see ``show vlan brief`` below
``S1#show vlan brief``|**only** displays VLANs, statuses, names, and assigned ports
``S1#show interface vlan [id]``|display detailed status for a specific VLAN's SVI (switched virtual interface) — its line/protocol status, IP address (if one is configured), and MTU. Useful for verifying inter-VLAN routing or remote-management IP setup on a Layer 3 switch
``S1#show interfaces trunk``|display trunking status for all trunk interfaces on the switch: whether each is actually trunking, its native VLAN, and which VLANs are allowed vs. currently active on it
...
``S1(config-if)#switchport mode access``|set the port to access mode — required before assigning the access and voice VLANs below
``S1(config-if)#switchport access vlan [vlan-id]``|assign the port's regular **access (data) VLAN** — this is the VLAN used by a PC connected behind the phone, if any
...
``S1(config-if)#switchport voice vlan [vlan-id]``|assign a voice VLAN to that port. The switch sends a CDP message telling the phone to tag its own traffic with this VLAN ID, so voice and data stay logically separated on the one physical cable
...
``S1(config-line)#login local ``|require login authentication against the local username database (the users created with ``username [admin] secret [ccna]`` above) rather than a shared line password
``S1(config-line)#exit``|exit VTY line configuration mode, returning to global configuration mode
