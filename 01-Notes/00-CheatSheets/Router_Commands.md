...
``R1#show history``|displays the buffer of recently entered commands for the current session. Default buffer size is 10 lines — change it with ``terminal history size [lines]`` (or ``history size [lines]`` from a line configuration mode)
...
``R1#show ip route``|displays the full IPv4 routing table — every route the router currently knows, from every source (directly connected, static, and any dynamic routing protocols), along with each route's administrative distance and metric. Additionally, issue ``show ip route ospf`` to see only the OSPF-learned networks (entries) on the routing table.
...
``R1(config-std-nacl)#``|the resulting prompt for standard named ACL configuration mode. From here, add entries with ``permit`` or ``deny`` followed by an address and wildcard mask — no ACL number needed, since the name identifies the list. Naming an ACL also lets you edit or remove individual entries later (e.g. ``no permit 10.0.0.0 0.0.0.255``) without deleting and rebuilding the whole list, unlike a numbered ACL
...
``R1(config-if)#ip nat inside``|marks this interface as the NAT **inside** interface — the one facing your private/local network. NAT only translates traffic that crosses between a marked inside and a marked outside interface
``R1(config)#interface [int-id]``|enter configuration mode for the _outside_ interface
``R1(config-if)#ip nat outside``|marks this interface as the NAT **outside** interface — typically the one facing the ISP/public internet
...
``access-list [number] {permit \| deny} [source] [wildcard mask]``|create a standard ACL matching the private-side source address(es) that are eligible for dynamic translation — this ACL isn't applied to an interface like a normal ACL; it's only referenced by the NAT command below to define which traffic gets translated
``ip nat inside source list [acl-number] pool [pool name]``|bind the ACL (the "who gets translated") to the pool (the "what they get translated to"), completing the dynamic NAT configuration
